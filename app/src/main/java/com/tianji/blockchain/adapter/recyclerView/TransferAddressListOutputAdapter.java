...
public class TransferAddressListOutputAdapter extends BasicItemClickRecyclerViewAdapter<Transaction.Output> {

    public TransferAddressListOutputAdapter(Context context, List<Transaction.Output> data) {
    ...
    protected void onBindContentViewHolder(@NonNull RecyclerView.ViewHolder viewHolder, int position) {
        ...
        holder.tv_address.setText(data.get(position).getAddresses()[0]);
        holder.tv_value.setText(MathUtils.doubleKeep8(data.get(position).getValue()) + " BTC");
    }
